...
DEFINE_string(map, "src/navigation/experiment.txt", "Name of vector map file");
...
/*
  ...
  for(int i = 0; i < (int)msg.ranges.size(); i++) {
    if(msg.ranges[i] > msg.range_max || msg.ranges[i] < msg.range_min)
      continue;
    Vector2f point(msg.ranges[i]*cos(msg.angle_min + msg.angle_increment*i) + 0.2f, 
    msg.ranges[i]*sin(msg.angle_min + msg.angle_increment*i));
    point_cloud_.push_back(point);
  }
  navigation_->UpdatePointCloud(point_cloud_);
  last_laser_msg_ = msg;
*/
 }
